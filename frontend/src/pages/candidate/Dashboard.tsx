...
export default function Dashboard() {
  ...
  const handleSyncProfile = async () => {
    ...
    try {
      ...
      const WTTJ_SVC = `http://${window.location.hostname}:8012`
      let done = false
  ...
  const handleCreateNewAccount = async () => {
    ...
    try {
      ...
      if (response.data?.success || response.status === 202) {
        ...
        let done = false
        const WTTJ_SVC = `http://${window.location.hostname}:8012` // Live status polling bypasses API gateway for immediate feedback
